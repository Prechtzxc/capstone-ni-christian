...
import { Calendar, Plus, Clock, Users, MoveHorizontal, X, Loader2 } from "lucide-react"
import { ReserveDialog } from "@/components/reserve-dialog"
import { Dialog, DialogContent, DialogTitle, DialogDescription } from "@/components/ui/dialog"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"

// Binalik natin 'yung static data mo for now para hindi mag-crash!
const dummyEventVenues = [
  { id: "v1", name: "Conference Hall", capacity: 80, price: "₱300.00/hr", image: "/images/venue-interior.jpg", description: "Experience our Conference Hall with panoramic views.", amenities: ["Projector", "Sound System", "AC"], category: "event" },
  { id: "v2", name: "Garden Pavilion", capacity: 100, price: "₱350.00/hr", image: "/images/venue-chandelier.png", description: "Beautiful outdoor pavilion for your events.", amenities: ["Outdoor", "Lighting", "Catering Area"], category: "event" },
  { id: "v3", name: "Grand Ballroom", capacity: 200, price: "₱500.00/hr", image: "/images/venue-interior.jpg", description: "Spacious ballroom for grand celebrations.", amenities: ["AC", "Stage", "Dance Floor"], category: "event" },
  { id: "v4", name: "Rooftop Terrace", capacity: 120, price: "₱450.00/hr", image: "/images/venue-chandelier.png", description: "Stunning city views from our rooftop.", amenities: ["Outdoor", "Bar Area", "Lounge"], category: "event" },
  { id: "v5", name: "The Milestone Event Place", capacity: 80, price: "₱400.00/hr", image: "/images/venue-interior.jpg", description: "Perfect for milestones and intimate gatherings.", amenities: ["AC", "Sound System", "Decorations"], category: "event" },
]

const dummyOfficeSpaces = [
  { id: "o1", name: "Private Office A", capacity: 5, price: "₱150.00/hr", image: "/images/venue-interior.jpg", description: "Quiet and private space for focused work.", amenities: ["AC", "High-speed WiFi", "Whiteboard"], category: "office" },
  { id: "o2", name: "Co-working Space", capacity: 20, price: "₱200.00/hr", image: "/images/venue-chandelier.png", description: "Collaborative environment with great lighting.", amenities: ["AC", "Shared Pantry", "WiFi"], category: "office" },
]
...
export default function BookingsPage() {
  ...
  // Mock Fetching Logic (Ready to be replaced by Supabase)
  useEffect(() => {
    const fetchVenues = async () => {
      try {
        ...
        // ==========================================
        // TO-DO: Replace with your actual Supabase query later
        ...
        // Simulating network request para hindi mag-JSON error
        setTimeout(() => {
          const allDummyVenues = [...dummyEventVenues, ...dummyOfficeSpaces]
          setVenues(allDummyVenues)
          setSelectedVenue(allDummyVenues[0].id) // Default to first venue (v1)
        ...
        <DialogContent className="!max-w-[1200px] w-[95vw] h-[90vh] p-0 overflow-hidden flex flex-col md:flex-row gap-0 border-0 bg-black [&>button]:hidden shadow-2xl">
          ...
          {isLoadingVenues ? (
            ...
            <>
              ...
              {/* ========================================== */}
              <div className="w-full md:w-[300px] lg:w-[350px] flex-shrink-0 flex flex-col bg-slate-50 border-t md:border-t-0 md:border-l border-gray-200 overflow-hidden">
                ...
                <Tabs className="flex-1 flex flex-col overflow-hidden" defaultValue="events">
                  ...
                  <TabsContent className="flex-1 overflow-y-auto p-4 space-y-3 m-0 custom-scrollbar" value="events">
                    {eventVenuesList.length === 0 && <p className="text-xs text-gray-500 text-center py-4">No event venues available.</p>}
                  ...
                  <TabsContent className="flex-1 overflow-y-auto p-4 space-y-3 m-0 custom-scrollbar" value="offices">
                    {officeSpacesList.length === 0 && <p className="text-xs text-gray-500 text-center py-4">No office spaces available.</p>}
                ...
                {/* Bottom Footer: Amenities & Buttons (COMPACT VERSION) */}
                <div className="p-4 bg-white border-t border-gray-200 shrink-0 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
                  <div className="mb-3">
                  ...
                  <div className="flex gap-2 mt-1">
                    <Button 
                      variant="outline" 
                      onClick={() => setShowVenueModal(false)}
                      className="flex-1 rounded-lg border-gray-300 font-medium hover:bg-gray-50 text-slate-700"
                    >
                      Close
                    </Button>
                    <Button 
                      onClick={handleProceedToBooking}
                      className="flex-1 rounded-lg bg-[#0f172a] hover:bg-slate-800 text-white font-medium shadow-md"
                    >
                      Book Now
                    </Button>
                  </div>
