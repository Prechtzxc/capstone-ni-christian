...
export type Message = {
  ...
  timestamp: Date
  isRead: boolean
  imageUrl?: string // FIX: Dinagdag natin ito para sa Attachments
}
...
interface ChatContextType {
  messages: Message[]
  sendMessage: (text: string, senderOverride?: "user" | "bot" | "admin", targetId?: string, skipAutoReply?: boolean, imageUrl?: string) => void
  unreadCount: number 
...
export function ChatProvider({ children }: { children: ReactNode }) {
  ...
  // FIX: In-update ang parameter para isama yung imageUrl
  const sendMessage = useCallback((text: string, senderOverride?: "user" | "bot" | "admin", targetIdOverride?: string, skipAutoReply: boolean = false, imageUrl?: string) => {
    ...
    const newMessage: Message = {
      ...
      timestamp: new Date(),
      isRead: false,
      imageUrl // Isinama natin sa message object yung image!
    }

    setMessages(prev => [...prev, newMessage])
    if (activeSender !== "bot") sendSocketMessage(newMessage)

    if (activeSender === "user" && !autoReplyLock.current && !skipAutoReply) {
