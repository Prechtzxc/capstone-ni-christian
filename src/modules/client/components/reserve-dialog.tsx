...
import { Clock, ChevronLeft, ChevronRight, ArrowRight, Loader2, Calendar as CalendarIcon, PartyPopper, CheckCircle2, AlertCircle } from "lucide-react"
...
export function ReserveDialog({ open, onOpenChange, selectedVenueId = "v1", onBackToVenues, editingBooking, onSubmitSuccess }: ReserveDialogProps) {
  ...
  }, [date, allBookings, editingBooking, selectedVenueId])
  ...
  const getDayStatus = (d: number) => {
    ...
    // FIX: CHINE-CHECK NI CLIENT KUNG YUNG CURRENT VENUE NIYA BA AY NAKA MAINTENANCE SA ARAW NA 'TO
    if (maintenanceDates?.includes(`${selectedVenueId}|${iterDateStr}`)) return "maintenance"
            ...
            <>
              ...
              <div className="flex-1 bg-white p-6 md:p-8 flex flex-col justify-center relative overflow-y-auto max-h-[90vh] rounded-b-[2rem] md:rounded-r-[2rem] md:rounded-bl-none">
                <div className="w-full max-w-[500px] mx-auto animate-in slide-in-from-right-8">
                  <div className="mb-5"><h2 className="text-2xl font-black text-slate-900 tracking-tight flex items-center gap-2"><PartyPopper className="w-6 h-6 text-[#ea580c]" /> Event Details</h2><p className="text-xs text-slate-500 mt-1">Fill out your information to complete the request.</p></div>
                  <form onSubmit={handlePreSubmit} className="space-y-4">
                     ...
                     <div className="space-y-1.5">
                        ...
                        <Textarea value={notes} onChange={e => setNotes(e.target.value)} placeholder="Tell us more about your event layout, catering needs, etc." className="bg-white border-slate-200 min-h-[60px] resize-none rounded-lg p-3 text-xs focus-visible:ring-[#ea580c] shadow-sm" />
                     </div>
      ...
      <Dialog open={showConfirmAlert} onOpenChange={setShowConfirmAlert}>
         <DialogContent className="sm:max-w-[450px] rounded-[2rem] p-6 z-[99999]">
            ...
            <div className="space-y-2 bg-white p-4 rounded-xl border border-slate-200 text-slate-800 mt-2">
               <div className="flex justify-between border-b border-slate-100 pb-2"><span className="text-slate-500 font-medium">Event:</span><span className="font-bold truncate max-w-[200px] text-right">{eventName}</span></div>
               <div className="flex justify-between border-b border-slate-100 pb-2"><span className="text-slate-500 font-medium">Venue:</span><span className="font-bold text-right">{venueMap[selectedVenueId || "v1"]}</span></div>
               <div className="flex justify-between border-b border-slate-100 pb-2"><span className="text-slate-500 font-medium">Date & Time:</span><span className="font-bold text-right">{date}<br/>{startTime ? formatTime(parseFloat(startTime)) : ""} - {endTime ? formatTime(parseFloat(endTime)) : ""}</span></div>
               <div className="flex justify-between"><span className="text-slate-500 font-medium">Guests:</span><span className="font-bold">{guests} pax</span></div>
            </div>
